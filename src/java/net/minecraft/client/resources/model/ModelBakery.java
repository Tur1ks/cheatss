...
public class ModelBakery {
    ...
    public void bakeModels(ModelBakery.TextureGetter pTextureGetter) {
        this.topLevelModels.forEach((locIn, modelIn) -> {
            ...
            try {
                bakedmodel = new ModelBakery.ModelBakerImpl(pTextureGetter, locIn).bakeUncached(modelIn, BlockModelRotation.X0_Y0);
    ...
    public void loadItemModelAndDependencies(ResourceLocation pModelLocation) {
        ...
        String s = pModelLocation.getPath();
        if (s.startsWith("optifine/") || s.startsWith("item/")) {
            resourcelocation = pModelLocation.withPrefix("");
        }
    ...
    class ModelBakerImpl implements ModelBaker {
        ...
        ModelBakerImpl(final ModelBakery.TextureGetter pTextureGetter, final ModelResourceLocation pModelLocation) {
            this.modelTextureGetter = materialIn -> pTextureGetter.get(pModelLocation, materialIn);
        }
        ...
        BakedModel bakeUncached(UnbakedModel pModel, ModelState pState) {
            return this.bakeUncached(pModel, pState, this.modelTextureGetter);
        }
