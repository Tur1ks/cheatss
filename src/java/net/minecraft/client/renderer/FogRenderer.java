    ...
    public static void setupColor(Camera pActiveRenderInfo, float pPartialTicks, ClientLevel pLevel, int pRenderDistanceChunks, float pBossColorModifier) {
        ...
        if (fogtype == FogType.WATER) {
            ...
            Vec3 vec34 = CubicSampler.sampleM(vec33, (xIn, yIn, zIn) -> {
                ...
                vec3m.fromRgbM(i2);
                return pLevel.effects().getBrightnessDependentFogColor(vec3m, f11);
            });
        ...
        if (fogtype == FogType.WATER) {
            ...
            Vec3 vec32 = CustomColors.getUnderlavaColor(pLevel, entity2.getX(), entity2.getY() + 1.0, entity2.getZ());
            if (vec32 != null) {
        ...
        if (Reflector.ForgeHooksClient_getFogColor.exists()) {
            Vector3f vector3f = (Vector3f)Reflector.ForgeHooksClient_getFogColor
                .call(pActiveRenderInfo, pPartialTicks, pLevel, pRenderDistanceChunks, pBossColorModifier, fogRed, fogGreen, fogBlue);
            fogRed = vector3f.x();
            fogGreen = vector3f.y();
            fogBlue = vector3f.z();
        }
