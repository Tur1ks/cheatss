...
public class EffectInstance implements Effect, AutoCloseable {
    ...
    public EffectInstance(ResourceProvider pResourceProvider, String pName) throws IOException {
        ResourceLocation resourcelocation = ResourceLocation.withDefaultNamespace("shaders/program/" + pName + ".json");
        if (Reflector.MinecraftForge.exists()) {
            resourcelocation = ResourceLocation.tryParse(pName).withPath(p -> "shaders/program/" + p + ".json");
        }
    ...
    public static EffectProgram getOrCreate(ResourceProvider pResourceProvider, Program.Type pType, String pName) throws IOException {
        ...
        if (program != null && !(program instanceof EffectProgram)) {
            ...
            if (program == null) {
                ResourceLocation resourcelocation = ResourceLocation.withDefaultNamespace("shaders/program/" + pName + pType.getExtension());
                if (Reflector.MinecraftForge.exists()) {
                    resourcelocation = ResourceLocation.tryParse(pName).withPath(p -> "shaders/program/" + p + pType.getExtension());
                }
