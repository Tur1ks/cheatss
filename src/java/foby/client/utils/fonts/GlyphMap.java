...
public class GlyphMap {
    ...
    public void destroy() {
        Foby.mc.getInstance().getTextureManager().release(this.bindToTexture);
        this.glyphs.clear();
        this.width = -1;
        this.height = -1;
        generated = false;
    ...
    public static void registerBufferedImageTexture(ResourceLocation i, BufferedImage bi) {
        try {
            ...
            if (RenderSystem.isOnRenderThread()) {
                Foby.mc.getInstance().getTextureManager().register(i, tex);
            } else {
                RenderSystem.recordRenderCall(() -> Foby.mc.getInstance().getTextureManager().register(i, tex));
            }
        } catch (Throwable e) {
            e.printStackTrace();
        }
