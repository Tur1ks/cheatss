...
public class ColorUtil {
    ...
    public static int astolfo(int speed, int offset, float saturation, float brightness, float alpha) {
        ...
        float hueNormalized;
        return DrawHelper.reAlphaInt(Color.HSBtoRGB((double) ((hueNormalized = hue % 360.0F) / 360.0F) < 0.5 ? -(hueNormalized / 360.0F) : hueNormalized / 360.0F, saturation, brightness), Math.max(0, Math.min(255, (int) (alpha * 255.0F))));
    }
    ...
    public static int getColorStyle(int s) {
        return Foby.themesUtil.getCurrentStyle().getColor(s);
    }
}
